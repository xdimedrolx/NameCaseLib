 ...
 * @package NameCaseLib
 */
class NameCaseCore extends NCL
{
    /**
     * Версия библиотеки
     * @var string
    ...
    /**
     * Массив содержит елементы типа NameCaseWord. Это все слова которые нужно обработать и просклонять
     * @var array
     */
    private $words = array();
     ...
     * @return bool   если правило было задествовано, тогда true, если нет - тогда false
     */
    ...
    /**
     * В массив <var>$this->words</var> добавляется новый об’єкт класса NameCaseWord
    ...
    public function setFirstName($firstname = '')
    {
        if ($firstname) {
            ...
            $this->words[$index] = new NameCaseWord($firstname);
    ...
    public function setSecondName($secondname = '')
    {
        if ($secondname) {
            $index = count($this->words);
            $this->words[$index] = new NameCaseWord($secondname);
    ...
    public function setFatherName($fathername = '')
    {
        if ($fathername) {
            $index = count($this->words);
            $this->words[$index] = new NameCaseWord($fathername);
     ...
     * @param NameCaseWord $word слово которое нужно идентифицировать
     */
    private function prepareNamePart(NameCaseWord $word)
    {
        if (!$word->getNamePart()) {
     ...
     * @param NameCaseWord $word слово для которого нужно определить пол
     */
    private function prepareGender(NameCaseWord $word)
    {
    ...
    private function splitFullName($fullname)
    {
        ...
        foreach ($list as $word) {
            $this->words[] = new NameCaseWord($word);
        }
     ...
     * @param NameCaseWord $word слово, которое нужно просклонять
     */
    private function WordCase(NameCaseWord $word)
    {
        ...
        foreach ($cur_words as $k => $cur_word) {
            ...
            $o_ncw = new NameCaseWord($cur_word);
     ...
     * @param  NameCaseWord $word   слово для котрого нужно вернуть падеж
     * @param  int          $number номер падежа, который нужно вернуть
     * @return mixed        массив или строка с нужным падежом
     */
    private function getWordCase(NameCaseWord $word, $number = null)
    {
    ...
    /**
     * Возвращает внутренний массив $this->words каждая запись имеет тип NameCaseWord
     ...
     * @return bool true - если было использовано правило из списка, false - если правило не было найденым
     */
     ...
     * @return bool true - если слово было успешно изменено, false - если не получилось этого сделать
     */
     ...
     * @param NameCaseWord $word обьект класса слов, для которого нужно определить пол
     */
    protected function GenderByFirstName(NameCaseWord $word)
    {
    }
     ...
     * @param NameCaseWord $word обьект класса слов, для которого нужно определить пол
     */
    protected function GenderBySecondName(NameCaseWord $word)
    {
    }
     ...
     * @param NameCaseWord $word обьект класса слов, для которого нужно определить пол
     */
    protected function GenderByFatherName(NameCaseWord $word)
    {
    }
     ...
     * - <b>S</b> - фамилия
     * - <b>F</b> - отчество
     * @param NameCaseWord $word обьект класса слов, который необходимо идентифицировать
     */
    protected function detectNamePart(NameCaseWord $word)
    {
    }
