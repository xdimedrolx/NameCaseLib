...
use NCL\NCL\NameCaseCore;
use NCL\NCL\NameCaseWord;
 ...
 * @package NameCaseLib
 */
class NameCaseUa extends NameCaseCore
{
     ...
     * </ul>
     * @return bool true - якщо було задіяно правило з переліку, false - якщо правило не знайдено
     */
        ...
        if ($this->Last(1) == 'р') {
            if ($this->inNames($this->workingWord, array('Ігор', 'Лазар'))) {
                ...
                if (NCLStr::substr($osnova, -2, 1) == 'і') {
                    $osnova = NCLStr::substr($osnova, 0, NCLStr::strlen($osnova) - 2).'о'.NCLStr::substr($osnova,
                            -1, 1);
                }
    ...
    protected function manRule3()
    {
        ...
        if ($this->in($this->Last(1), $this->consonant.'оь')) {
            ...
            if ($osLast != 'й' and NCLStr::substr($osnova, -2,
                    1) == 'і' and !$this->in(NCLStr::substr(NCLStr::strtolower($osnova), -4, 4),
                    array('світ', 'цвіт')) and !$this->inNames($this->workingWord,
                    'Гліб') and !$this->in($this->Last(2), array('ік', 'іч'))) {
                $osnova = NCLStr::substr($osnova, 0, NCLStr::strlen($osnova) - 2).'о'.NCLStr::substr($osnova, -1,
                        1);
            }


            //Випадання букви е при відмінюванні слів типу Орел
            if (NCLStr::substr($osnova, 0, 1) == 'о' and $this->FirstLastVowel($osnova,
                    $this->vowels.'гк') == 'е' and $this->Last(2) != 'сь') {
                $delim = NCLStr::strrpos($osnova, 'е');
                $osnova = NCLStr::substr($osnova, 0, $delim).NCLStr::substr($osnova, $delim + 1,
                        NCLStr::strlen($osnova) - $delim);
            }
            ...
            if ($group == 1) {
                ...
                elseif ($this->in($this->Last(2), array('ов', 'ев', 'єв')) and !$this->inNames($this->workingWord,
                        array('Лев', 'Остромов'))) {
                    $this->wordForms($osnova, array(
                        $osLast.'а',
                        $osLast.'у',
                        $osLast.'а',
                        $osLast.'им',
                        $osLast.'у',
                        $this->inverse2($osLast).'е',
                    ), 1);
                    $this->Rule(302);
                    return true;
    ...
    protected function womanRule3()
    {
        ...
        //Ті що на ськ
        if ($this->Last(1) == 'а' and ($this->in($this->Last(2, 1), 'чнв') or $this->in($this->Last(3, 2),
                    array('ьк')))) {
            $this->wordForms($this->workingWord, array(
                $BeforeLast.'ої',
                $BeforeLast.'ій',
     ...
     * @return bool true - якщо слово успішно змінене, false - якщо невдалося провідміняти слово
     */
     ...
     * @param NameCaseWord $word об’єкт класу зі словом, для якого необхідно визначити стать
     */
     ...
     * - <b>S</b> - прізвище
     * - <b>F</b> - по-батькові
     * @param NameCaseWord $word об’єкт класу зі словом, яке необхідно ідентифікувати
     */
    protected function detectNamePart(NameCaseWord $word)
    {
        ...
        //Похоже на имя
        if ($this->in($this->Last(3), array('тин' /* {endings_sirname3} */)) or $this->in($this->Last(4),
                array('ьмич', 'юбов', 'івна', 'явка', 'орив', 'кіян' /* {endings_sirname4} */))) {
            $first += 0.5;
        }
